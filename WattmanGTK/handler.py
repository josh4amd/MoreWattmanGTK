...
def execute_apply(window):
    ...
    # Add password entry field to the dialog
    password_entry = Gtk.Entry()
    password_entry.set_visibility(False)  # Hide entered characters
...
def verify_password(password):
    ...
    test_command = "echo 'verifying'"
    sudo_command = f"echo '{password}' | sudo -S {test_command}"
...
def run_command_as_sudo(window, password_entry, commands_file):
    ...
    if verify_password(password):
        ...
        dialog = Gtk.MessageDialog(
            ...
            text="Commands to be run (RUN AT YOUR OWN RISK):",
        )
